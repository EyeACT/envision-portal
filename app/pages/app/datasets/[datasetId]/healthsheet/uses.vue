...
const { data, error } = await useFetch(
  `/api/datasets/${datasetId}/healthsheet/uses`,
  {},
);
...
if (data.value) {
  ...
  state.records =
    data.value.uses.records.length > 0
      ? data.value.uses.records.map((subject: any) => ({
          id: subject.id,
  ...
  state.version = data.value.uses.version;
...
async function onSubmit(event: FormSubmitEvent<typeof state>) {
  ...
  try {
    const res = await $fetch(`/api/datasets/${datasetId}/healthsheet/uses`, {
      body: b,
      method: "PUT",
    });
    console.log(res);
    ...
    <div class="flex-1 overflow-y-auto p-4 pb-28 space-y-6">
      <UBreadcrumb
        ...
        :items="[
          ...
          {
            label: 'Healthsheet - Uses',
            to: `/app/datasets/${datasetId}/healthsheet/uses`,
          },
        ]"
      />

      <UForm
        id="healthsheet-uses-form"
        :validate="validate"
        :state="state"
        class="space-y-6"
        @submit="onSubmit"
      >
        <div class="flex w-full flex-wrap items-center justify-between rounded-lg bg-white p-6 shadow-sm dark:bg-gray-900">
          <div class="flex w-full items-center justify-between gap-3">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Uses</h1>
          </div>
