...
const { data, error } = await useFetch(
  `/api/datasets/${datasetId}/healthsheet/collection`,
  {},
);
...
async function onSubmit(event: FormSubmitEvent<typeof state>) {
  ...
  try {
    const res = await $fetch(`/api/datasets/${datasetId}/healthsheet/collection`, {
      body: b,
      method: "PUT",
    });
    console.log(res);
...
<template>
  <div class="flex flex-col h-[calc(100vh-6rem)] relative overflow-hidden">
      ...
      <UBreadcrumb
        ...
        :items="[
          ...
          { label: data?.title, to: `/app/datasets/${datasetId}` },
          {
            ...
            to: `/app/datasets/${datasetId}/healthsheet/collection`,
          },
        ]"
      />

      <UForm
        id="healthsheet-collection-form"
        :validate="validate"
        :state="state"
        class="space-y-6"
        @submit="onSubmit"
      >
        <div class="flex w-full flex-wrap items-center justify-between rounded-lg bg-white p-6 shadow-sm dark:bg-gray-900">
          <div class="flex w-full items-center justify-between gap-3">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Collection</h1>
          </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 z-10 px-6 pb-6 bg-gradient-to-t from-gray-50 via-gray-50/90 to-transparent pt-4 dark:from-gray-950">
      <UButton
        form="healthsheet-collection-form"
        type="submit"
