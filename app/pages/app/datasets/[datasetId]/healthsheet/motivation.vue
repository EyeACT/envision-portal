...
const { data, error } = await useFetch(
  `/api/datasets/${datasetId}/healthsheet/motivation`,
  {},
);
...
if (data.value) {
  ...
  state.records =
    data.value.motivation.records.length > 0
      ? data.value.motivation.records.map((subject: any) => ({
          id: subject.id,
  ...
  state.version = data.value.motivation.version;
...
async function onSubmit(event: FormSubmitEvent<typeof state>) {
  ...
  try {
    const res = await $fetch(`/api/datasets/${datasetId}/healthsheet/motivation`, {
      body: b,
      method: "PUT",
    });
    console.log(res);
...
<template>
  <div class="flex flex-col h-[calc(100vh-6rem)] relative overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-4 pb-28 space-y-6">
      <UBreadcrumb
        ...
        :items="[
          ...
          {
            label: 'Healthsheet - Motivation',
            to: `/app/datasets/${datasetId}/healthsheet/motivation`,
          },
        ]"
      />

      <UForm
        id="healthsheet-motivation-form"
        :validate="validate"
        :state="state"
        class="space-y-6"
        @submit="onSubmit"
      >
        <div class="flex w-full flex-wrap items-center justify-between rounded-lg bg-white p-6 shadow-sm dark:bg-gray-900">
          <div class="flex w-full items-center justify-between gap-3">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Motivation</h1>
          </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 z-10 px-6 pb-6 pt-4">
      <UButton
        form="healthsheet-motivation-form"
        type="submit"
