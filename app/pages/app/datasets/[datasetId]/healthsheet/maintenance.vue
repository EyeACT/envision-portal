...
const { data, error } = await useFetch(
  `/api/datasets/${datasetId}/healthsheet/maintenance`,
  {},
);
...
if (data.value) {
  ...
  state.records =
    data.value.maintenance.records.length > 0
      ? data.value.maintenance.records.map((subject: any) => ({
          id: subject.id,
  ...
  state.version = data.value.maintenance.version;
...
async function onSubmit(event: FormSubmitEvent<typeof state>) {
  ...
  try {
    const res = await $fetch(`/api/datasets/${datasetId}/healthsheet/maintenance`, {
      body: b,
      method: "PUT",
    });
    console.log(res);
...
<template>
  <div class="flex flex-col h-[calc(100vh-6rem)] relative overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-4 pb-28 space-y-6">
      <UBreadcrumb
        ...
        :items="[
          ...
          {
            label: 'Healthsheet - Maintenance',
            to: `/app/datasets/${datasetId}/healthsheet/maintenance`,
          },
        ]"
      />

      <UForm
        id="healthsheet-maintenance-form"
        :validate="validate"
        :state="state"
        class="space-y-6"
        @submit="onSubmit"
    ...
    <div class="absolute bottom-0 left-0 right-0 z-10 px-6 pb-6 bg-gradient-to-t from-gray-50 via-gray-50/90 to-transparent pt-4 dark:from-gray-950">
      <UButton
        form="healthsheet-maintenance-form"
        type="submit"
