      ...
      <UForm :state="state" :validate="validate" class="flex flex-col gap-6" @submit.prevent="onSubmit">
          ...
          <div class="flex w-full flex-col gap-4">
            ...
            <div class="flex w-full gap-4">
              ...
              <UFormField label="Family Name" name="familyName" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorFamilyName" placeholder="Leonhart" class="w-full" />
              </UFormField>
            </div>

            <UFormField label="Title" name="title" class="w-full">
              <UInput v-model="state.responsiblePartyInvestigatorTitle" placeholder="Warrior Candidate" class="w-full" />
            </UFormField>

            <div class="flex w-full gap-4">
              <UFormField label="Affiliation" name="affiliation" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorAffiliationName" placeholder="Marleyan Military" class="w-full" />
              </UFormField>

              <UFormField label="Affiliation Identifier" name="affiliationId" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorAffiliationIdentifier" placeholder="0156zyn36" class="w-full" />
              </UFormField>
            </div>

            <div class="flex w-full gap-4">
              <UFormField label="Affiliation Identifier Scheme" name="affiliationScheme" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorAffiliationIdentifierScheme" placeholder="ROR" class="w-full" />
              </UFormField>

              <UFormField label="Affiliation Identifier Scheme URI" name="affiliationSchemeUri" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorAffiliationIdentifierSchemeUri" placeholder="https://ror.org" class="w-full" />
              </UFormField>
            </div>

            <div class="flex w-full gap-4">
              <UFormField label="Identifier Scheme" name="idScheme" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorIdentifierScheme" placeholder="ORCID" class="w-full" />
              </UFormField>

              <UFormField label="Identifier Value" name="idValue" class="w-full">
                <UInput v-model="state.responsiblePartyInvestigatorIdentifierValue" placeholder="0000-0003-2829-8032" class="w-full" />
              </UFormField>
            </div>
          </div>
        </div>
        ...
        <div class="flex w-full flex-wrap items-center justify-between rounded-lg bg-white p-6 shadow-sm dark:bg-gray-900">
          <div class="flex w-full flex-col gap-4">
            ...
            <div class="flex w-full gap-4">
              <UFormField label="Identifier Scheme" name="leadSponsorScheme" class="w-full">
                <UInput v-model="state.leadSponsorIdentifierScheme" placeholder="ROR" class="w-full" />
              </UFormField>
        ...
        <div class="flex w-full flex-wrap items-center justify-between rounded-lg bg-white p-6 shadow-sm dark:bg-gray-900">
          <div class="flex w-full flex-col gap-4">
            ...
            <UFormField name="collaborators">
              <CardCollapsible v-for="(item, index) in visibleCollaborators" v-show="!item.deleted" :key="item.id" class="my-1 shadow-none" :title="item.name || `Collaborator ${index + 1}`" bordered>
                <template #header-extra>
                  <UButton icon="i-lucide-trash" label="Remove collaborator" variant="soft" color="error" @click="() => removeCollaboratorById(item.id)" />
                </template>

                <div class="flex flex-col gap-3">
                  <UFormField :name="`collaborators[${index}].name`" label="Name">
            ...
            <UButton icon="i-lucide-plus" variant="outline" color="primary" label="Add Collaborator" @click="addCollaborator" />
          </div>
        </div>
